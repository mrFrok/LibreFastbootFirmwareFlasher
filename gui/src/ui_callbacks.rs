...
fn export_log(model: &VecModel<LogEntry>, tab_name: &str) -> Option<String> {
    ...
    let fname = format!("lfff-{}-log-{}.txt", tab_name, now.format("%Y-%m-%d_%H-%M-%S"));
    if let Some(path) = rfd::FileDialog::new()
        .set_file_name(&fname)
        .add_filter("Text", &["txt"])
        .save_file() {
            let mut content = String::new();
            content.push_str(&format!("LFFF {} Log Export — {}\n", tab_name, now.format("%Y-%m-%d %H:%M:%S")));
...
pub fn poll(
    ...
    while let Ok(m) = rx.try_recv() {
        ...
        if let WMsg::DlProgress { ref downloaded, .. } = m
            && downloaded.is_empty() { *last_dl_pct = 0; }

        match m {
            WMsg::Log { level, message, tab } => add_log(models, &ui, &level, tab, &message),
            WMsg::Progress { fraction, partition } => {
                ui.set_flash_progress(fraction);
                ui.set_current_partition(partition.clone().into());
                if !partition.is_empty() { ui.set_flash_status(partition.into()); }
            }
            WMsg::DeviceDetected { name, serial, slot, is_fastboot_mode } => {
                ui.set_device(DeviceInfo { connected: true, name: name.into(), serial: serial.into(), slot: slot.into(), is_fastboot_mode });
            }
            WMsg::DeviceDisconnected => {
                ui.set_device(DeviceInfo { connected: false, name: "\u{2014}".into(), serial: "\u{2014}".into(), slot: "\u{2014}".into(), is_fastboot_mode: false });
            }
            ...
            WMsg::DlProgress { percent, speed, eta, downloaded, total, raw_line } => {
                ...
                if let Some(ref msg) = dl_log {
                    ...
                    if !is_bar { add_log(models, &ui, &LogLevel::Info, 1, &raw_line); }
                }
            }
            ...
            WMsg::CableTestProgress { step, total, status } => {
                ...
                if step >= total { ui.set_cable_test_passed(true); }
            }
...
pub fn register_callbacks(
    ...
    let t = ctx.clone();
    ui.on_check_device(move || { t.send(Cmd::CheckDevice).ok(); });
    ...
    let t = ctx.clone();
    ui.on_reboot_to(move |target| { t.send(Cmd::RebootTo(target.to_string())).ok(); });

    let w = ui.as_weak();
    ui.on_set_scale(move |scale| {
        save_scale(scale);
        if let Some(ui) = w.upgrade() { ui.set_pending_scale(scale); }
    });
    ...
    let t = ctx.clone();
    ui.on_run_cable_test(move || { t.send(Cmd::CableTest).ok(); });
    ...
    let t = ctx.clone();
    ui.on_cancel_download(move || { t.send(Cmd::CancelDownload).ok(); });
    ...
    let t = ctx.clone();
    ui.on_check_deps(move || { t.send(Cmd::CheckDeps).ok(); });

    let t = ctx.clone();
    ui.on_install_deps(move || { t.send(Cmd::InstallDeps).ok(); });

    let t = ctx.clone();
    ui.on_run_driver_test(move || { t.send(Cmd::DriverTest).ok(); });

    let t = ctx.clone();
    ui.on_get_device_info(move || { t.send(Cmd::CheckDevice).ok(); });

    let t = ctx.clone();
    ui.on_post_flash_reboot(move || { t.send(Cmd::PostFlashReboot).ok(); });

    let t = ctx.clone();
    ui.on_post_flash_wipe(move || { t.send(Cmd::PostFlashWipe).ok(); });

    let t = ctx.clone();
    ui.on_download_firmware(move |u| { t.send(Cmd::Download { url: u.to_string() }).ok(); });
    ...
    ui.on_paste_clipboard(move || {
        if let Some(ui) = w.upgrade() {
            ...
            if let Some(t) = text {
                ...
                if !trimmed.is_empty() { ui.set_download_url(trimmed.into()); }
            }
        }
    });
    ...
    let t = ctx.clone();
    ui.on_set_active_slot(move |slot| { t.send(Cmd::SetActiveSlot { slot: slot.to_string() }).ok(); });

    let t = ctx.clone();
    ui.on_check_for_updates(move || { t.send(Cmd::CheckForUpdates).ok(); });
