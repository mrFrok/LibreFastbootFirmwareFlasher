...
/// Display firmware ARB info and prompt user for confirmation if ARB is active.
/// Returns true if the user confirmed to proceed (or if no confirmation is needed).
pub fn arb_confirmation_gate(firmware_arb: &ArbInfo) -> bool {
    ...
    println!(
        ...
        "Firmware :".dimmed(),
        fw_ver.bold(),
        format!("({})", firmware_arb.source).dimmed()
    );

    match firmware_arb.version {
        Some(0) | None => {
            let msg = if firmware_arb.version == Some(0) {
                "ARB = 0 — hard ARB not enforced on this firmware."
            } else {
                "ARB version unknown — proceeding."
            };
            println!("  {}  {}", "✓".green(), msg);
        ...
        Some(v) => {
            ...
            println!(
                "  {}",
                "You will NOT be able to downgrade to firmware with a lower ARB afterwards.".dimmed()
            );
        }
    }
